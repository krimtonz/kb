...
void gfx_init(void){
    ...
    kfont->texture = resource_get(R_KB_FONT);
    kfont->c_width = 8;
    kfont->c_height = 8;
    kfont->cx_tile = 2;
    kfont->cy_tile = 16;    
...
void gfx_draw_sprite(gfx_texture *texture, int x, int y, int tile, int width, int height){
    gfx_draw_sprite_color(texture, x, y, tile, width, height, COLOR_DEFAULT);
}
...
void gfx_printf(uint16_t left, uint16_t top, const char *format, ...){
    ...
    gfx_printf_va_color(left, top, COLOR_DEFAULT, format, args);
    va_end(args);
}
