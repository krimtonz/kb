...
static void key_draw(menu_item_t *item){
    ...
    if(c == 0x01){
        gfx_draw_sprite(resource_get(R_KB_ICON), menu_item_x(item), menu_item_y(item), 5, 8, 8);
    }else if(c == 0x08){
        gfx_draw_sprite(resource_get(R_KB_ICON), menu_item_x(item), menu_item_y(item), 6, 8, 8);
    }else{
...
void menu_keyboard_get(menu_item_t *item, char **dest){
    ...
    menu_trigger_event(&kb.main_menu, MENU_EVENT_ENTER, &keyboard_data);
}